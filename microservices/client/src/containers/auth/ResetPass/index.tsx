...
export const ForgotPass = () => {
  ...
  const modalFooter = emailSent ? null : (
    <a
      onClick={() => {
  ...
  const handleForgotPass = async (e: SyntheticEvent) => {
    ...
    await sendPostRequest({
      endpoint: "/api/server/forgot_pass",
  ...
  const resetPasswordContent = () => {
    return (
      <div>
        <h6 className="mb-3">
          Enter the email associated with you CCG inventory account here:
        </h6>
        <Form noValidate onSubmit={handleForgotPass}>
          ...
          />
          <Button type="submit" id="send-email" className="mt-2">
            Send email
          </Button>
        </Form>
        <p className="mt-5 text-center">
          If we find your email in our records, we'll send you an email with
          next steps.
        </p>
      </div>
    );
      ...
      <div>
        ...
        <div className="mt-5">
          ...
          <p className="text-center">
            Keep an eye out for an incoming email with your next steps.
          </p>
        </div>
      </div>
    );
    ...
    <>
      <div className="mt-5 mb-5">
        ...
        <div className={`d-flex justify-content-start ${styles.flex_wrapper}`}>
          ...
          <Link className={`ps-1 ${styles.link_header}`} to={"/register"}>
            {" "}
            Create an account
          </Link>
        </div>
      ...
      <Form noValidate onSubmit={handleSignIn}>
        <Form.Group>
          ...
          <Form.Control
            ...
            required
            onChange={(e) => {
              _.set(formData, keyPaths.email, e.target.value);
            }}
            onBlur={(data) => {
          ...
          {error.email?.valid === false && (
            <Form.Control.Feedback data-testid="email-error" type="invalid">
              {error.email.message}
        ...
        <Row>
          <div
            className={
              "d-flex justify-content-between" + ` ${styles.flex_wrapper}`
            }
          >
